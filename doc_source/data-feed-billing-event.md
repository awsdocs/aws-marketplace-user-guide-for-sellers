...
| from\_account\_id |  The account that initiated the billing event\. If `transaction_type` is `SELLER_REV_SHARE`, it is the buyer's payer account\. This is a foreign key to the [account](data-feed-account.md) data feed\. Can be used to join to the `Account` data feed on the `account_id` field\.  | 
